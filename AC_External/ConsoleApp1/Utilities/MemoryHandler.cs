...
namespace AC_External
{
    public class MemoryHandler : IDisposable
    {
        ...
        public MemoryHandler(int pID, Process process)
        {
            Console.WriteLine(pID + process.ProcessName);
            m_hProcess = OpenProcess(ProcessAccessFlags.VMRead | ProcessAccessFlags.VMWrite | ProcessAccessFlags.VMOperation, true, pID);
        ...
        public byte[] Read(int MemoryAddress, uint bytesToRead)
        {
            ...
            IntPtr procHandle = OpenProcess(ProcessAccessFlags.VMRead | ProcessAccessFlags.VMWrite | ProcessAccessFlags.VMOperation, true, m_ReadProcess.Id);
            if (procHandle == IntPtr.Zero)
            {
                return new byte[0];
            }
            ReadProcessMemory(procHandle, (IntPtr)MemoryAddress, buffer, bytesToRead, out IntPtr ptrBytesReaded);
        ...
        public byte[] PReadMem(IntPtr MemoryAddress, uint bytesToRead, out int bytesReaded)
        {
            byte[] buffer = new byte[bytesToRead];

            ReadProcessMemory(m_hProcess, MemoryAddress, buffer, bytesToRead, out IntPtr ptrBytesReaded);
        ...
        public int ReadMultiLevelPointer(int MemoryAddress, uint bytesToRead, Int32[] offsetList)
        {
            ...
            for (int i = 0; i < (offsetList.Length); i++)
            {
                if (i == 0)
                {
                    ReadProcessMemory(procHandle, (IntPtr)pointerAddy, btBuffer, (uint)btBuffer.Length, out lpOutStorage);
                }
        ...
        {
            if (ReadMem(MemoryAddress, 1, out byte[] buffer) == 0)
            {
                return new byte();
            }
            else
        ...
        public unsafe byte PReadByte(int* MemoryAddress)
        {
            if (ReadMem(*MemoryAddress, 1, out byte[] buffer) == 0)
            {
                return new byte();
        ...
        {
            if (ReadMem(MemoryAddress, 4, out byte[] buffer) == 0)
            {
                return 0;
            }
            else
        ...
        public unsafe int PReadInt(int* MemoryAddress)
        {
            if (ReadMem(*MemoryAddress, 1, out byte[] buffer) == 0)
            {
                return 0;
        ...
        public unsafe uint PReadUInt(int* MemoryAddress)
        {
            if (ReadMem(*MemoryAddress, 1, out byte[] buffer) == 0)
            {
                return 0;
        ...
        {
            if (ReadMem(MemoryAddress, sizeof(float), out byte[] buffer) == 0)
            {
                return 0;
            }
            else
        ...
        public unsafe float PReadFloat(int* MemoryAddress)
        {
            if (ReadMem(*MemoryAddress, 1, out byte[] buffer) == 0)
            {
                return 0;
        ...
        public int WriteMem(int MemoryAddress, byte[] buf)
        {
            ...
            VirtualProtectEx(procHandle, (IntPtr)MemoryAddress, (uint)buf.Length, PAGE_READWRITE, out uint oldProtect);
            WriteProcessMemory(procHandle, (IntPtr)MemoryAddress, buf, (uint)buf.Length, out IntPtr ptrBytesWritten);
            CloseHandle(procHandle);
            return ptrBytesWritten.ToInt32();
        }

        public unsafe void PWriteMem(IntPtr MemoryAddress, byte[] bytesToWrite, out int bytesWritten)
        {
            WriteProcessMemory(m_hProcess, MemoryAddress, bytesToWrite, (uint)bytesToWrite.Length, out IntPtr ptrBytesWritten);
        ...
        public unsafe void PWriteByte(int* MemoryAddress, byte b)
        {
            WriteMem(*MemoryAddress, new byte[] { b });
        }
        #endregion
        ...
        public unsafe void PWriteInt(int* MemoryAddress, int w)
        {
            byte[] buf = BitConverter.GetBytes(w);
            WriteMem(*MemoryAddress, buf);
        }
        ...
        public unsafe void PWriteUInt(int* MemoryAddress, uint u)
        {
            byte[] buf = BitConverter.GetBytes(u);
            WriteMem(*MemoryAddress, buf);
        }
        ...
        public unsafe void PWriteFloat(int* MemoryAddress, float f)
        {
            byte[] buf = BitConverter.GetBytes(f);
            WriteMem(*MemoryAddress, buf);
        }
